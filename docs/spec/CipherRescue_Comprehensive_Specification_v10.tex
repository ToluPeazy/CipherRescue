...
\vfill
{\small\color{midgrey} Version 3.1 $\cdot$ 2026}
\end{titlepage}
...
\paragraph{Adversary scope.} The proof above is sound against an external attacker who modifies the log post-hoc relative to a trusted publication of $h_n$. The session key $K$ resides in the orchestrator process address space and is therefore visible to any code executing in that process; an operator able to compromise the orchestrator can construct any log they wish and publish the corresponding terminal hash. The audit log is therefore tamper-evident under adversary class A1 and partially under A3 (a supply-chain attacker who reaches the orchestrator at run time), but does not exclude fabrication by adversary class A2. This scoping is discussed in \S\ref{sec:auditscope}; v1.1+ mitigations are introduced in \S\ref{sec:audit:enhancements}. Marketing language describing the audit log as ``evidence-grade'' must be qualified by adversary class accordingly.

\paragraph{Session-key hygiene.} The session key $K$ is used to MAC both BackupTokens (Definition~\ref{def:backuptoken}) and audit-log entries. Single-key reuse across two protocol contexts is safe under HMAC-SHA256 today but violates good cryptographic hygiene. From v1.1, $K$ is treated as a master and per-purpose subkeys are derived via HKDF-SHA256:
\[
  K_{\mathrm{audit}} \leftarrow \mathrm{HKDF}(K,\, \texttt{"cipherrescue/audit/v1"}), \quad
  K_{\mathrm{backup}} \leftarrow \mathrm{HKDF}(K,\, \texttt{"cipherrescue/backup/v1"}).
\]

\section{Audit Log Enhancements}
\label{sec:audit:enhancements}

The hash-chained audit log defined in \S\ref{sec:auditlog:def} provides tamper evidence under adversary class A1. The properties it does \emph{not} provide --- robustness against fabrication by an operator who controls the orchestrator process --- are the subject of two v1.1+ enhancements introduced here. Both are optional in the sense that they require capabilities (a TPM, an external sink) not present in every recovery environment, and both are documented now so that procurement evaluations can correctly identify CipherRescue's evidence properties on each target deployment.

\paragraph{TPM-backed session signing (v1.1+, hardware-conditional).}
Where the recovery host has a TPM 2.0 device, the session signing identity is moved from a process-resident HMAC key to a TPM-resident signing key. At session start, CipherRescue generates an Ed25519 (or ECDSA P-256) signing key in a TPM volatile primary slot under a session-specific authorisation policy that binds the key to PCR values of the running ISO, requires presence of the operator at session start, and prohibits export. Audit-log entries are signed using the TPM key rather than HMAC'd under $K_{\mathrm{audit}}$. The genesis record additionally carries a TPM Quote attesting the boot-state PCRs, signed by the TPM's Endorsement-Key-derived Attestation Key.

The property gained: an operator cannot mint signatures without the TPM cooperating, and the TPM will not cooperate if the boot state has been tampered with. The property does not eliminate A2 entirely --- a sufficiently capable operator with physical access to a TPM-equipped host has known attack avenues (bus-sniffing on LPC-bus TPMs being the canonical example) --- but it raises the cost of fabrication from ``read process memory'' to ``mount a hardware attack against the TPM,'' which is a categorically different threat profile and outside the model for most insider-misuse scenarios.

\paragraph{External append-only audit sink (v1.1+, configuration-conditional).}
Where the recovery host has network reachability to a configured external sink, audit-log entries are streamed in real time to that sink, treated as the canonical record. The local on-disk log becomes a cache. Supported sinks are intentionally minimal: a USB device mounted with \texttt{O\_APPEND} only and physically removed after the session; an internal append-only file service (e.g., AWS CloudWatch Logs with stream immutability, an S3 bucket with Object Lock, or a Vector deployment terminating in a write-once destination); or, for forensic deployments, a hardware write-once medium (DVD-R, write-once removable storage) attached to a dedicated audit channel.

The property gained: terminal-hash publication is no longer dependent on operator transcription. The append-only sink itself becomes the publication channel, and any divergence between the local log and the sink is detectable. The configuration is opt-in --- v1.0 air-gapped recoveries continue to operate without it --- and the deployment documentation describes which combinations of TPM and external sink are recommended for which classes of asset.

\paragraph{Combined posture summary.} CipherRescue v1.0 ships with the hash-chained log and explicit scoping of its adversary properties (\S\ref{sec:auditscope}). v1.1+ adds TPM-backed signing where hardware permits and external append-only streaming where network configuration permits. Neither v1.1 enhancement is a guarantee against every insider scenario, but their combination raises the operator-fabrication cost from ``negligible'' to ``forensically detectable and operationally expensive'', which is the appropriate posture for an open-source recovery framework that does not control its deployment environment.
...
\section{Knowledge Base Construction and Data Provenance}
\label{sec:kb-provenance}

The entailment weight matrix $\rho : \calF \times \calU \to [0,1]$ constitutes the \emph{knowledge base} of the diagnostic engine. A central concern for any diagnostic system is the provenance and reliability of this knowledge base. Commercial organisations do not routinely disclose encryption failure events for competitive and security reasons, so the matrix cannot be derived from observed incident records. The knowledge base is instead constructed from three independently auditable sources.

\paragraph{Source 1 --- Published encryption scheme specifications.}
The structural signals --- \texttt{magic\_partial}, \texttt{checksum\_fail}, \texttt{header\_version\_mismatch}, \texttt{mbr\_absent}, \texttt{gpt\_corrupt}, \texttt{pba\_absent}, \texttt{crypt\_list\_partial} --- are derived from the published on-disk format specifications: the BitLocker sector layout and header structure \citep{ferguson2006}; the LUKS2 JSON metadata and integrity extension specification \citep{broz2018}; and the TCG Opal 2.0 standard \citep{tcg2015}. These specifications are public, authoritative, and version-stable. Entailment weights for structural signals are determined by logical necessity: a device with a corrupted LUKS2 header \emph{will} exhibit \texttt{checksum\_fail} by definitional consequence of the format. Weights are therefore set to $1.0$ (certain entailment) or $0.0$ (excluded by definition), with the exception of shared signals where the weight reflects mechanism analysis.

\paragraph{Source 2 --- Storage failure prediction literature.}
The physical degradation signals --- \texttt{smart\_reallocated}, \texttt{smart\_pending}, \texttt{smart\_timeout}, \texttt{io\_error\_lba} --- are calibrated from the storage reliability literature. Pinheiro et al.\ \citep{pinheiro2007} established SMART attributes 5, 187, 197, and 198 as the most predictive precursors of imminent drive failure from a large-scale observational study. The Backblaze 200,000-drive longitudinal dataset \citep{backblaze2024} provides the empirical frequency distribution used to calibrate the Health Score weights (Definition~\ref{def:healthscore}) and the entailment weights for physical signals in $\rho$. These weights represent the conditional probability of observing a given SMART signal pattern under a physical degradation failure mode --- a quantity estimable from large-scale observational data without access to commercially sensitive failure records.

\paragraph{Source 3 --- Synthetic device profiles.}
Validation uses synthetic device profiles with pre-specified, ground-truth failure modes. Each profile is constructed by combining a structurally valid device descriptor derived from Source~1 specifications with a SMART attribute vector drawn from the Source~2 distribution conditional on the intended failure mode, together with an injected corruption consistent with the target failure mode. Ground truth is known by construction, enabling exact validation of the diagnostic engine. The synthetic profile generator and the resulting benchmark corpus are research outputs of this project (Section~\ref{sec:empirical:stream1}) and are deposited as Zenodo artefacts \citep{babatunde2025zenodo}.

\begin{tcolorbox}[enhanced, colback=lightblue!40, colframe=royalblue, arc=4pt,
  fonttitle=\bfseries, title={Knowledge Base Independence from Commercial Disclosure}]
The three-source construction avoids dependence on commercially disclosed failure data entirely. Structural signal weights follow from published specifications by logical necessity. Physical signal weights are calibrated from peer-reviewed literature and large-scale observational datasets \citep{pinheiro2007,backblaze2024}. Validation uses synthetic profiles with known ground truth. The knowledge base is fully reproducible and independently auditable from public sources.
\end{tcolorbox}
...
\section{Diagnostic Rationale for Minimisation}
\label{sec:minimisation-rationale}

A natural question is why the diagnostic problem is framed as a \emph{minimisation}: finding the smallest cost-weighted set of failure mode hypotheses that accounts for all observed signals, rather than enumerating all consistent combinations. Three complementary arguments justify minimisation as the correct objective in this setting.

\paragraph{Parsimony as a diagnostic principle.}
The principle of diagnostic parsimony --- preferring the smallest sufficient explanation for observed evidence --- has a long precedent in systems diagnosis. Reiter's foundational theory of model-based diagnosis \citep{reiter1987} defines a \emph{diagnosis} as a minimal set of component abnormalities consistent with observations, explicitly invoking minimality as the criterion separating actionable explanations from over-explanations. De Kleer and Williams \citep{dekleer1987} apply the same criterion in the General Diagnostic Engine. CipherRescue adopts this principle directly: the minimal cover $C^*$ identifies the fewest failure mode hypotheses that jointly account for all signals observed on the device. Hypotheses not in $C^*$ are not asserted absent --- they are simply unsupported by the current evidence. This is the correct epistemic stance for a recovery tool operating under incomplete information, where over-commitment to unexplained hypotheses could direct recovery effort toward non-existent faults.

\paragraph{LP optimality as a certificate, not a heuristic.}
Minimality in CipherRescue is not achieved by a greedy heuristic but by LP optimisation, producing a \emph{certifiably} optimal cover. The duality gap $\delta = \mathrm{UB} - \mathrm{LB}$ is returned in the \texttt{FailureReport} and is zero for the large majority of FDE instances (Proposition~\ref{prop:tractable}), meaning the cover is provably optimal. The greedy alternative \citep{vazirani2001} provides only an $H_n$-approximation guarantee \citep{karp1972} --- adequate for many covering problems but insufficient when the diagnostic output justifies irreversible recovery actions on a device containing potentially unrecoverable data. The LP dual variables simultaneously provide evidence weights $\rho_{\mathrm{LP}}$ (Proposition~\ref{prop:dualreasons}), a by-product of optimisation that greedy and pure enumeration approaches do not supply.

\paragraph{Scope of the minimality claim.}
It is important to be explicit about what the minimal cover $C^*$ does and does not assert. It does not claim that \emph{only} the failure modes in $C^*$ could have produced the observed signals --- other combinations may be consistent with the evidence. What $C^*$ provides is the parsimonious working hypothesis under current observations, with LP-certified optimality and explicit dual-derived evidence weights. The full enumeration of all minimal covers --- equivalent to enumerating all minimal hitting sets of $\calU_{\mathrm{obs}}$ in the sense of \citet{reiter1987} --- is identified as a research extension for multi-device RAID correlation (Solution~\ref{sol:multidevicescpr}) and is a target for the journal version of the P1 paper. In the single-device recovery case, the cost-optimal cover with full dual annotation is the operationally appropriate output: a practitioner needs to act on one prioritised diagnosis, not navigate an enumerated set of alternatives.
...
This chapter analyses adversarial scenarios faced by CipherRescue at deployment. The treatment requires an explicit threat model so that subsequent edge-case solutions can be evaluated against a fixed set of assumptions rather than a moving target.
...
\begin{description}[noitemsep]
  \item[\textbf{A1 --- External attacker.}] An attacker without physical or authenticated access to the recovery host. Reaches the system over the network if any network surface is exposed; supplies malicious media (USB drives, ISOs) but does not control the operator. Default network adversary in the Dolev--Yao sense: can intercept, modify, replay, and inject network traffic on any reachable interface.
  \item[\textbf{A2 --- Partially-trusted operator.}] The recovery operator. Holds physical access, root privileges on the live image, and --- where escrow is configured --- credentials sufficient to retrieve recovery keys. The operator is trusted to follow procedure but is \emph{not} assumed to be the legitimate authorisor of every recovery action they are technically able to perform. This is the principal threat for insider data theft and for evidence integrity in legal contexts.
  \item[\textbf{A3 --- Supply-chain attacker.}] An attacker who compromises an upstream artefact: a Python dependency, an Alpine package, the signing key, the build-runner pool, or a community plugin. Operates before the recovery session begins; the live image itself is the attack vector.
\end{description}

\subsection{Trust Anchors}

CipherRescue's security properties rest on four trust anchors. Each is named so that its compromise consequences can be reasoned about.

\begin{description}[noitemsep]
  \item[\textbf{T1 --- The release signing identity.}] In v1.0 a GPG key; in v1.1+ a Sigstore identity backed by a transparency log (\S\ref{sec:isoverify:solution}). Signs the canonical ISO manifest. Compromise of T1 negates integrity guarantees on the live image.
  \item[\textbf{T2 --- The reproducible-build inputs.}] Pinned Python wheel hashes, pinned Alpine package digests, and the build-runner toolchain. Reproducibility (Definition~\ref{def:reprobuild}) holds the build function deterministic; T2 ensures the inputs are honest.
  \item[\textbf{T3 --- The session key $K$.}] Sampled per session from \texttt{/dev/urandom}. Authenticates BackupTokens (Definition~\ref{def:backuptoken}) and audit-log entries (Definition~\ref{def:auditlog}). Resides in the orchestrator process address space and is therefore visible to any code executing in that process.
  \item[\textbf{T4 --- Operator procedural compliance.}] CipherRescue's authorisation properties degrade gracefully against operator error and partially against operator malice. v1.1+ introduces compensating controls (escrow two-person rule, \S\ref{sec:escrow}) where asset value justifies the cost.
\end{description}

\subsection{Scope of the Audit-Trail Property}
\label{sec:auditscope}

Theorem~\ref{thm:tamper} establishes that any modification of an audit-log entry is detectable given a trusted external publication of the terminal hash $h_n$. The property holds against adversary class A1: an external attacker cannot rewrite the log in a way that survives verification against the published $h_n$.

The property does \emph{not} hold in full against A2. The session key $K$ resides in the operator-controlled orchestrator process; an operator able to compromise the orchestrator can construct any log they wish, compute the corresponding $h_n$, and publish that. The audit log is therefore tamper-evident against external attackers but not against insider fabrication. Section~\ref{sec:audit:enhancements} introduces v1.1+ controls --- TPM-backed session signing and external append-only streaming --- that close this gap on hardware that supports them.

This scoping is stated explicitly so that procurement and compliance evaluations of CipherRescue can be calibrated correctly. Descriptive language qualifying the audit log as ``evidence-grade'' must be read in conjunction with adversary class: evidence-grade against external tampering, residually mutable by a hostile session-key holder.

\subsection{Network Trust Posture}
\label{sec:nettrust}

CipherRescue v1.0 ships without any externally-reachable network surface: no listening services, no inbound management ports. NTP and NFS, where used, are outbound and operator-initiated. The BMC management LAN, when present, is treated as \emph{untrusted}: serial-over-LAN traffic must be assumed observable and modifiable by any party that has compromised the BMC. This posture drives the design decision in \S\ref{sec:headless} to defer remote management interfaces to v2.0 behind a security-review gate.

\section{ISO Verification UX Gap}

\begin{edgecase}{Operator Skips Signature Verification}{verifygap}
The GPG-signed ISO and SHA-256 manifest mitigate tampered image distribution, but only if operators verify the signature before booting. Under time pressure, operators frequently skip this step. A modified CipherRescue ISO that exfiltrates key material while displaying a normal-looking UI is indistinguishable to a non-verifying operator. A naive in-band mitigation --- displaying the running image's hash on the boot screen --- does not address this threat: the hash is computed by the running image, and a malicious image displays whatever hash it chooses. The operator class that skips out-of-band verification is the same class that accepts whatever the screen reports.
\end{edgecase}

\begin{solution}{Out-of-Band Verification via Transparency Log and Boot-Chain Pinning}{bootverify}
\label{sec:isoverify:solution}
Verification must occur \emph{before} the image runs. Three layered mechanisms are adopted, each operative outside the trust boundary of the ISO itself.

\begin{enumerate}[noitemsep]
  \item \textbf{Multi-channel manifest distribution (v1.0).} The signed manifest containing $H_{\mathrm{published}}$ is published simultaneously to the project website, a tagged GitHub release, and an independent mirror. An adversary compromising any single channel is detected by cross-channel comparison. Operators are instructed to verify against at least two channels.
  \item \textbf{Sigstore transparency log binding (v1.1+).} The release pipeline publishes the manifest signature to the Sigstore Rekor public transparency log. Each signed manifest is therefore associated with a Rekor inclusion proof verifiable against an append-only Merkle tree. Unauthorised signatures (e.g.\ from a compromised signing identity) appear in the log and are detectable by passive third-party monitoring. The release identity is bound to the GitHub Actions OIDC issuer, removing long-lived signing-key handling from the build process. This subsumes the v1.0 GPG flow without breaking it.
  \item \textbf{Secure Boot key pinning (v1.1+).} The CipherRescue shim is signed by a project-managed Secure Boot key that operators may pin in their firmware key database. A tampered image fails to boot rather than presenting a falsely reassuring verification screen. This control is opt-in --- enterprise environments with manageable firmware policy benefit; ad-hoc USB recovery does not.
\end{enumerate}

\paragraph{What is \emph{not} adopted.} An earlier draft of this specification proposed a boot-time SHA-256 display rendered by GRUB during the first five seconds of boot, alongside a QR code linking to verification instructions. This mitigation is removed: a malicious ISO renders the legitimate hash unchanged, and the QR code can point to attacker-controlled documentation. The operator's verification act has to occur before the bootloader executes. Recording \emph{whether} verification was confirmed remains in scope as a session-audit field, but is treated as operator self-attestation, not as a cryptographic property.
\end{solution}
...
A community plugin distributed via the \texttt{plugins/} directory with a valid \texttt{plugin.json} manifest is loaded and executed at root level. Two distinct threat shapes apply, and they require different responses. The first --- \emph{network exfiltration} --- is the threat addressed by sandboxing: a malicious plugin reaches a network endpoint over the live image's network stack to send out captured data. The second --- \emph{authorised-capability misuse} --- is the harder problem: by design, a plugin holding a valid \texttt{AuthToken} has the cryptographic capability to unlock the volume it is recovering, and is therefore handed legitimate access to plaintext. After that point, sandboxing of network or syscall surface does little to constrain what the plugin does with that plaintext --- it can be embedded in audit-log entries, written to the backup destination through legitimate file paths, or planted in unallocated sectors of the source drive for post-recovery persistence.
\end{edgecase}

\begin{solution}{Phased Plugin Trust Model}{pluginsandbox}
\label{sec:plugintrust}
The plugin trust model is implemented in two phases: a hardened sandbox in v1.0--v1.x that addresses the network-exfiltration threat, and a capability-mediation architecture in v2.0 that addresses the authorised-capability-misuse threat. Both are documented now so that v1.x deployments can be evaluated against a roadmap rather than against an end state.

\paragraph{v1.0--v1.x: Hardened sandbox (network-exfiltration defence).}
\begin{enumerate}[noitemsep]
  \item \textbf{Code signing with pre-load verification.} Community plugins must be signed with a key registered in the CipherRescue community keyring. The plugin registry verifies the signature on the plugin file \emph{before} \texttt{importlib} is invoked --- module-level code never executes from an unverified plugin. Operators may override with an explicit flag that is logged.
  \item \textbf{Network namespacing.} Plugins execute in a network namespace with no external connectivity (\texttt{unshare --net}). NTP and NFS operations are handled by the core framework outside the plugin sandbox.
  \item \textbf{Mount-namespace isolation.} The plugin runs in a mount namespace where the backup destination is not visible. Recovery output destined for the backup destination is written by the orchestrator on the plugin's behalf, through a narrow API.
  \item \textbf{Capability restriction.} A \texttt{seccomp} profile restricts plugins to a syscall whitelist required for disk operations, with the filter inherited across \texttt{execve} via \texttt{SECCOMP\_FILTER\_FLAG\_TSYNC} so that subprocess invocations of \texttt{cryptsetup}, \texttt{dislocker}, etc., remain constrained. Network syscalls (\texttt{socket}, \texttt{connect}, \texttt{sendto}) are blocked.
  \item \textbf{Plugin audit-log API constraint.} Plugin-emitted audit-log entries are constrained in size (4\,KB per entry) and structure (typed fields, no free-text payloads) to prevent the audit log being used as a covert exfiltration channel.
\end{enumerate}

\paragraph{v2.0: Capability mediation (authorised-capability defence).}
\label{sec:pluginv2}
The Layer~4 plugin contract is rewritten so that plugins do not receive direct access to plaintext volumes. The Safety Layer holds the unlocked block device and exposes a narrow capability-typed API: \texttt{read\_sectors(range)}, \texttt{replace\_header(bytes)}, \texttt{verify\_signature\_at(offset)}, and a small set of similar primitives. Plugins compose recovery actions from these primitives. Two consequences follow: a malicious plugin cannot read the entire plaintext volume into its address space (it can only request bounded sector ranges, all of which are logged), and the orchestrator can attest at session close to the exact set of bytes the plugin observed and the exact set of bytes it requested be written --- a property that is useful both for forensic defensibility and for detecting plugin misbehaviour. This is a meaningful Layer~4 refactor and is correctly scheduled for v2.0; the v1.x sandbox is the deployed enforcement until v2.0 ships.

\paragraph{Trust tier distinction.} Throughout v1.x and v2.0, \emph{core plugins} (the bundled BitLocker, LUKS, VeraCrypt, Opal SED handlers, audited in-tree) and \emph{community plugins} (operator opt-in, screen-warning before load) are presented in the TUI as security-distinct, not equivalent. Operators see explicit provenance for every plugin invoked.
\end{solution}
...
A rack-mounted server accessed via IPMI/iDRAC/iLO has no physical display. The current TUI assumes a terminal. Headless recovery is a common enterprise scenario but introduces security exposure that must be sized against the recovery use case: BMC management LANs in modern enterprises routinely host third-party hardware and contractor devices, and BMC firmware itself has a documented vulnerability history (see \S\ref{sec:nettrust}). Any remote management surface inherits that exposure.
\end{edgecase}

\begin{solution}{Serial Console (v1.0); Remote Management Deferred to v2.0}{headlessolution}
A single headless mode is supported in v1.0--v1.x:

\begin{description}[noitemsep]
  \item[\textbf{Serial console mode (v1.0).}] The bootloader activates a serial console (\texttt{console=ttyS0,115200}) alongside the VGA console. The TUI is rendered in plain-text mode on serial. This is bootloader-configurable at ISO build time. Operators are explicitly informed in the deployment documentation that serial-over-LAN traffic must be assumed observable by any party that has compromised the BMC; recovery sessions involving high-value assets should not be conducted over compromised or untrusted BMC paths.
\end{description}

\paragraph{Web management interface --- deferred to v2.0.} An earlier draft of this specification proposed a Flask-based web management interface for v1.1+, exposing the TUI workflow as a JSON API over the BMC's network interface, authenticated by a session token displayed on the serial console. The interface is removed from the v1.x roadmap. A privileged JSON-and-HTML control plane on the BMC LAN reachable from a third-party browser is a substantial attack surface (DNS rebinding, CSRF, cross-origin abuse, BMC-side passive capture, session-token replay) that the project is not yet equipped to harden to a defensible standard for forensic recovery tooling.

The interface is revisited in v2.0 behind an explicit security-review gate. Any v2.0 design will be required to address, at minimum: (i) mandatory mTLS termination with an out-of-band-distributed server certificate fingerprint; (ii) bind-interface allow-listing; (iii) \texttt{Host}-header validation as DNS-rebinding defence; (iv) CSRF protection on every state-changing endpoint; (v) session-token rotation per privileged action; (vi) opt-in at ISO build time, never enabled by default; (vii) an updated threat model in which the BMC LAN is treated as an attacker-controlled network. Until those design properties are present and reviewed, headless deployments use serial console only.
\end{solution}
...
In enterprise environments, BitLocker recovery keys are stored in Active Directory / Microsoft Entra ID, LUKS passphrases in HashiCorp Vault, and Opal SED credentials in CyberArk. Requiring operators to copy credentials manually introduces error and security risk. However, automated escrow retrieval introduces an authorisation problem of its own: possession of an escrow credential is not the same property as authorisation to recover a specific drive. Under adversary class A2 (\S\ref{sec:threatmodel}), an operator with read-access to organisational recovery keys can use an automated escrow path to unlock any drive belonging to that organisation that physically reaches them, regardless of whether the recovery is authorised --- and the audit log will faithfully record the action as legitimate operator activity. The escrow design must therefore distinguish credential possession from recovery authorisation explicitly.
\end{edgecase}

\begin{solution}{Phased Escrow Model with Two-Person Default}{escrowadapter}
\label{sec:escrow:solution}
Escrow is implemented in three phases. The phasing is documented now so that v1.0 deployments are not assumed to support functionality that is scheduled for later releases.

\paragraph{v1.0: Manual operator entry only.}
Recovery credentials are entered by the operator at the TUI prompt. No automated escrow path is shipped. The audit log records the credential type entered (e.g., ``BitLocker recovery key'', ``LUKS passphrase'') without recording the credential itself. Memory-zeroing protocol per Solution~\ref{sol:memlocking} applies.

\paragraph{v1.1+: Two-person-rule escrow as default.}
An \texttt{EscrowAdapter} abstract interface is added to the plugin contract. Bundled adapters:

\begin{itemize}[noitemsep]
  \item \texttt{ActiveDirectoryEscrow}: retrieves BitLocker recovery keys from Active Directory via authenticated LDAP query using the drive's GUID
  \item \texttt{VaultEscrow}: retrieves secrets from HashiCorp Vault via the KV~v2 API
  \item \texttt{MSAADEscrow}: retrieves BitLocker keys from Microsoft Entra ID via the Graph API
\end{itemize}

By default, every escrow retrieval requires \emph{two-person authorisation}: the primary operator initiates the retrieval; a second authorised party must authenticate through a separate channel (a hardware token, a co-present second operator, or a workflow approver in the organisation's identity provider) before the adapter releases the credential to the plugin. The two-person mode is the shipping default; single-operator escrow is selectable only with an explicit configuration flag that is permanently recorded in the audit log and in the session report. Credentials retrieved via escrow are never displayed to the operator --- they are passed directly to the plugin's authenticate method via memory-locked file-descriptor handoff (Solution~\ref{sol:memlocking}) and are zeroed immediately after the AuthToken is issued.

\paragraph{Enterprise extension: Ticket-bound escrow (v1.1+, optional).}
For organisations with case-management workflows, the escrow adapter can be configured to require a ticket reference (e.g., a ServiceNow incident ID) that is logged into both the upstream identity provider's audit trail and the CipherRescue session audit log, with cryptographic linkage between the two records. This binds the recovery action to a documented authorising case and addresses the disposition of the action under organisational governance, not just under cryptographic authentication. The extension is documented as an integration pattern rather than as bundled code, since the case-management endpoint is necessarily organisation-specific.

\paragraph{What this does and does not solve.} The two-person rule materially raises the cost of insider abuse: a single compromised operator credential is no longer sufficient to unlock organisational drives at scale. It does not eliminate insider risk in scenarios where two operators collude. The ticket-binding extension addresses the audit-defensibility dimension --- whether a recovery was \emph{procedurally} authorised, distinct from whether it was \emph{cryptographically} authorised. Enterprises adopting CipherRescue for high-value asset recovery should configure both controls.
\end{solution}
